...
public class BubbleService extends Service {
    ...
    private void _invia() {
        String urlBase = prefs.getString("url_attivo", "");
        String token = prefs.getString("token_attivo", "");
        ...
        new Thread(() -> {
            try {
                ...
                _mainThread(() -> {
                    if (code >= 200 && code < 400) { _toast("\u2713 Inviato"); _setC(0xFF34a853); new Handler(Looper.getMainLooper()).postDelayed(this::_rip, 2000); }
                    else { _toast("Errore HTTP " + code); _setC(0xFFea4335); }
                });
                conn.disconnect();
