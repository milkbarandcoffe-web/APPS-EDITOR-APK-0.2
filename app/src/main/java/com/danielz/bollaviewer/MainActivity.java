...
public class MainActivity extends Activity {
    ...
    private void aggiornaXButton() {
        int alpha = prefs.getInt("x_alpha", 180);
        xButton.setText("\u2715");
        xButton.setTextSize(18);
        xButton.setTextColor(0xFFFFFFFF);
        xButton.setAlpha(alpha / 255f);
        xButton.setGravity(Gravity.CENTER);
    }

    private void checkGasBanner(WebView view) {
        String js = "(function(){var btns=document.querySelectorAll('button,a,[role=button]');for(var i=0;i<btns.length;i++){var r=btns[i].getBoundingClientRect();if(r.right>window.innerWidth*0.75&&r.top<150&&r.width<80) return 'found';}return 'not found';})()";
        view.evaluateJavascript(js, result -> runOnUiThread(() -> {
            boolean b = result != null && result.contains("found");
            xButton.setAlpha(b ? 0.12f : (prefs.getInt("x_alpha", 180) / 255f));
            xButton.setClickable(!b);
        }));
    }

    private void tapX() {
        webView.evaluateJavascript(
            "(function(){var b=document.querySelectorAll('button,a,[role=button]');for(var i=0;i<b.length;i++){var t=b[i].textContent||'';var a=b[i].getAttribute('aria-label')||'';if(t.trim()=='\u00d7'||t.trim()=='\u2715'||a.toLowerCase().includes('close')||a.toLowerCase().includes('dismiss')){b[i].click();return 'chiuso';}}return 'nessun_banner';})()",
