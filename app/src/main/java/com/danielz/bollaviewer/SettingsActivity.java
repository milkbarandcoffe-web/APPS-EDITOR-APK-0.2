...
public class SettingsActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle s) {
        ...
        bS.setOnClickListener(v->{String u=euU.getText().toString().trim(),t=euT.getText().toString().trim();if(u.isEmpty()){Toast.makeText(this,"URL vuoto",Toast.LENGTH_SHORT).show();return;}_saveUrl(u,t);euU.setText("");euT.setText("");_updList(listaUrlLayout);Toast.makeText(this,"Salvato",Toast.LENGTH_SHORT).show();});
        root.addView(bS);
        ...
        root.addView(_sep());
        root.addView(_l("Testo bolla"));EditText eT=_i(prefs.getString("bolla_testo","B"));root.addView(eT);
        root.addView(_l("Colore bolla (hex es. #1a73e8)"));EditText eC=_i(String.format("#%06X",0xFFFFFF&prefs.getInt("bolla_colore",0xFF1a73e8)));root.addView(eC);
        ...
        bA.setOnClickListener(v->{SharedPreferences.Editor ed=prefs.edit();ed.putString("bolla_testo",eT.getText().toString().trim());try{ed.putInt("bolla_colore",Color.parseColor(eC.getText().toString().trim()));}catch(Exception ignored){}ed.putInt("bolla_alpha",sB.getProgress());ed.putInt("x_alpha",sX.getProgress());ed.apply();Toast.makeText(this,"Salvato. Riavvia bolla.",Toast.LENGTH_SHORT).show();finish();});
        root.addView(bA);sv.addView(root);setContentView(sv);
    }

    private void _saveUrl(String url,String tok){Set<String>l=new LinkedHashSet<>(prefs.getStringSet("url_lista",new LinkedHashSet<>()));l.add(url+"|||"+tok);prefs.edit().putStringSet("url_lista",l).putString("url_attivo",url).putString("token_attivo",tok).apply();}
    private void _updList(LinearLayout c){
        c.removeAllViews();Set<String>l=prefs.getStringSet("url_lista",new LinkedHashSet<>());String au=prefs.getString("url_attivo","");
        for(String e:l){String[]p=e.split("\|\|\|",2);String u=p[0],t=p.length>1?p[1]:"";LinearLayout r=new LinearLayout(this);r.setOrientation(LinearLayout.HORIZONTAL);TextView tv=new TextView(this);String lb=u.length()>38?"\u2026"+u.substring(u.length()-36):u;tv.setText((u.equals(au)?"\u25b6 ":"   ")+lb);tv.setTextColor(u.equals(au)?0xFF34a853:0xFFAAAAAA);tv.setTextSize(12);tv.setLayoutParams(new LinearLayout.LayoutParams(0,ViewGroup.LayoutParams.WRAP_CONTENT,1f));r.addView(tv);Button bU=new Button(this);bU.setText("USA");bU.setTextSize(10);bU.setOnClickListener(v->{prefs.edit().putString("url_attivo",u).putString("token_attivo",t).apply();_updList(c);});r.addView(bU);Button bD=new Button(this);bD.setText("\u2715");bD.setTextColor(0xFFea4335);bD.setTextSize(10);bD.setOnClickListener(v->{Set<String>n=new LinkedHashSet<>(prefs.getStringSet("url_lista",new LinkedHashSet<>()));n.remove(e);prefs.edit().putStringSet("url_lista",n).apply();if(u.equals(au))prefs.edit().putString("url_attivo","").putString("token_attivo","").apply();_updList(c);});r.addView(bD);c.addView(r);}
    }
