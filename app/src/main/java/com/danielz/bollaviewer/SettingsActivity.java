...
public class SettingsActivity extends Activity {
    ...
    private void _updList(LinearLayout c) {
        ...
        for (String e : l) {
            int idx = e.indexOf(SEP);
            String u = idx >= 0 ? e.substring(0, idx) : e;
            String t = idx >= 0 ? e.substring(idx + SEP.length()) : "";
            ...
            r.addView(bU);
            Button bD = new Button(this); bD.setText("X"); bD.setTextColor(0xFFea4335); bD.setTextSize(10);
            bD.setOnClickListener(v -> {
                Set<String> n = new LinkedHashSet<>(prefs.getStringSet("url_lista", new LinkedHashSet<>()));
                n.remove(e); prefs.edit().putStringSet("url_lista", n).apply();
                if (u.equals(au)) prefs.edit().putString("url_attivo", "").putString("token_attivo", "").apply();
                _updList(c);
            });
